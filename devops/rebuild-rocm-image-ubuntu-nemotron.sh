...
# rebuild-rocm-image-ubuntu-nemotron.sh - Clean rebuild of ROCm Nemotron Docker image
# Deletes the old image, clears builder cache, and rebuilds with tag ubuntu-llama-cpp-nemotron-omni:latest
# Run from project root: ./devops/rebuild-rocm-image-ubuntu-nemotron.sh

# Get the project root (parent of devops directory)
PROJECT_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$PROJECT_ROOT"

IMAGE_TAG="ubuntu-llama-cpp-nemotron-omni:latest"
DOCKERFILE="Dockerfile.ubuntu-rocm-nemotron-omni"

echo "========================================"
echo "ROCm Nemotron Image Rebuild"
echo "========================================"
